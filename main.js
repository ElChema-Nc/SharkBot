...
mobile: MethodMobile, 
browser: opcion == '1' ? ['SharkBot', 'Edge', '2.0.0'] : methodCodeQR ? ['SharkBot', 'Edge', '2.0.0'] : ['Chrome (Linux)', '', ''],
auth: {
creds: state.creds,
...
console.log(chalk.bgBlack(chalk.bold.redBright(`CONFIGURAR ARCHIVO ${chalk.bold.greenBright("config.js")} SU NÚMERO DE WHATSAPP NO TIENE CÓDIGO DE PAÍS, ${chalk.bold.yellowBright("EJEMPLO: +50582828282")}`)))
process.exit(0)
}} else {
while (true) {
addNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`POR FAVOR, ESCRIBA EL NÚMERO DE WHATSAPP QUE SERÁ BOT.\n${chalk.bold.yellowBright("CONSEJO: COPIE EL NÚMERO DE WHATSAPP Y PÉGUELO EN LA CONSOLA.")}\n${chalk.bold.yellowBright("EJEMPLO: +50582828282")}\n${chalk.bold.magentaBright('---> ')}`)))
...
if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', "SharkJadiBot"], tmp.forEach(filename => cp.spawn('find', [filename, '-amin', '2', '-type', 'f', '-delete'])))}, 30 * 1000)}
...
if (reason == 405) {
await fs.unlinkSync("./SharkBotSession/" + "creds.json")
...
let prekey = []
let directorio = readdirSync("./SharkBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./SharkBotSession/${files}`)
})
} 

function purgeSessionSB() {
try {
const listaDirectorios = readdirSync('./SharkJadiBot/');
let SBprekey = [];
listaDirectorios.forEach(directorio => {
if (statSync(`./SharkJadiBot/${directorio}`).isDirectory()) {
const DSBPreKeys = readdirSync(`./SharkJadiBot/${directorio}`).filter(fileInDir => {
return fileInDir.startsWith('pre-key-')
})
SBprekey = [...SBprekey, ...DSBPreKeys];
DSBPreKeys.forEach(fileInDir => {
if (fileInDir !== 'creds.json') {
unlinkSync(`./SharkJadiBot/${directorio}/${fileInDir}`)
}})
}})
...
const directories = ['./SharkBotSession/', './SharkJadiBot/']
